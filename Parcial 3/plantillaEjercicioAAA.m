    ...
    if evtAct.type=='A'
        ...
        for j =1:cantidadNodos
            ...
            if matrLinks(evtAct.value,j) < inf
                ...
                %Si ya llegué al destino entonces se debe detener la
                %simulación y se debe pintar el camino recorrido.
                if j == destino
                    parar =1;
                    actual = [actual j];
                ...
                elseif parar ==0 && ~any( arrayfun( @(x) isequal( j, x ), ruta_vecinos ) ) 
                    ...
                    %Aquí se le añade el tiempo de enviar un paquete desde
                    %un nodo a otro.
                    
                    newEvt.t=t+ matrLinks(evtAct.value,j)*3.333;
                    
                    %Se crea un nuevo evento tipo A y este recorrerá sus
                    %vecinos y sus vecinos recorrerán sus vecinos y así
...
M = containers.Map(actual ,padre);
pintarNodo = destino;
%for z=1:length(actual)
